...
STATIC = {
    1: ("A PLACE TO BEGIN", "Say the thing you have been rehearsing in your head.", "Pakistan-focused relationship guidance and conversation practice for adult women.", "maroon"),
    ...
    18: ("SPEAK NATURALLY", "Choose the language that helps you speak honestly.", "English, Urdu, Roman Urdu, and natural English-Urdu mixed language are available.", "blush"),
    ...
    24: ("KNOW BEFORE YOU SHARE", "Read the Terms before sharing personal details.", "Conversations are stored for the retention period selected in your account. Avoid details you do not need to share.", "cream"),
...
def render_editorial_variants():
    variants = {
        15: (
            "woman-writing-opening.png",
            "START WITH ONE SENTENCE",
            "You do not need a finished explanation.",
        ),
        17: (
            "couple-calm-conversation.png",
            "FAMILY ADVICE. COUPLE DECISION.",
            "Ask which input helps and which decision belongs inside your household.",
        ),
    }
    for day, (filename, title, body) in variants.items():
        source = Path(__file__).resolve().parent / "editorial" / filename
        canvas = cover_image(source, FEED).convert("RGBA")
        overlay = Image.new("RGBA", FEED, (0, 0, 0, 0))
        overlay_draw = ImageDraw.Draw(overlay)
        overlay_draw.rounded_rectangle((42, 55, 710, 830), radius=58, fill=(61, 15, 38, 224))
        overlay_draw.rounded_rectangle((82, 690, 600, 786), radius=48, fill=(241, 177, 91, 255))
        canvas = Image.alpha_composite(canvas, overlay)
        draw = ImageDraw.Draw(canvas)
        draw.text((86, 100), "DilSe", font=font(SERIF_BOLD, 60), fill=COLOURS["paper"])
        letterspaced(draw, (88, 180), "PAKISTAN  ·  CONVERSATIONS", font(SANS, 16), COLOURS["soft_saffron"], 2)
        text_block(draw, title, (86, 265, 540, 250), SERIF_BOLD, 66, 44, COLOURS["paper"], 13)
        text_block(draw, body, (88, 540, 530, 115), SANS, 35, 27, "#F4E8E8", 10)
        draw.text((341, 738), "START AT BAATDILSE.COM", font=font(SANS, 23), fill=COLOURS["dark_jam"], anchor="mm")
        draw.text((72, 1270), f"DAY {day:02d}  ·  EDITORIAL IMAGE", font=font(SANS, 18), fill=COLOURS["paper"])
        canvas.convert("RGB").save(OUT / f"day-{day:02d}" / "editorial-feed.png", quality=95)
